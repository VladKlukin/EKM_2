...
void Menu::showMainMenu() {
    // Отображает главное меню приложения и обрабатывает ввод пользователя
    // Запускает соответствующие режимы: ввод вручную, загрузка из файла, генерация
...
bool Menu::inputFromKeyboard() {
    // Считывает количество гнезд и для каждого гнезда считывает число соседей,
    // количество писем и номера соседей. Все данные проходят валидацию.
...
bool Menu::inputFromFile() {
    // Загружает структуру дерева из файла с указанным форматом.
    // Производит валидацию прочитанных значений и проверяет связность дерева.
    char filename[256];
...
bool Menu::inputRandom() {
    // Генерирует дерево из M вершин с рандомным количеством писем (0..2) и
    // гарантирует, что структура будет связным деревом (каждая вершина i>1
    // соединяется с случайным предыдущим узлом в [1..i-1]).
